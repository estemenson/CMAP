...
#from steve.model.releaseModel import ReleaseModel
try:
...
class ReleaseMinViewOld(MyInnerWindowWithSaveAndTrash):
    def __init__(self,wnd,ctrl, **kwargs):
        kwargs.setdefault('name','')
        kwargs.setdefault('minimized_label', kwargs['name'])
        super(ReleaseMinViewOld,self).__init__(**kwargs)
        if ctrl is None:
        ...
        sz=(self._full_width - int(len('Release Name: ')*pixels),
                                                            self.font_height)
        self.story_name = MyTextInput(label=self.name,id='release_name_id',
        ...
        self.row0.add_widget(MTLabel(label='Release Name:'))
    ...
    def close(self, touch):
        ...
        super(ReleaseMinViewOld, self).close(touch)
